...
\imagen{LLL_Evolution}{Cronología de la evolución de los LLM.~\cite{Tolaka}}{1}
...
\imagen{Timeline_LLM2}{Cronología de los modelos lingüísticos de gran tamaño (superior a 10B) de los últimos años.~\cite{zhao2023survey}}{1}
...
\imagen{Word2Vec}{Ilustración de un espacio de incrustación 2D tal que el vector que une ``lobo'' con ``perro'' es el mismo que el vector que une ``tigre'' con ``gato''.~\cite{Chollet}}{0.5}
...
\imagen{transformer_model_architecture-f}{Arquitectura de los modelos de redes neuronales 
 profundas \textit{Transformers}.~\cite{Nvidia_Transformers}}{1}
...
\imagen{OpenAI_Tokenizer}{Muestra de un texto y su equivalente en tokens usando el tokenizador de OpenAI para GPT-3.5 y GPT-4.~\cite{OpenAI_Tokenizer}}{1}
...
\imagen{Byte-pair_Encoding}{Ejemplo de codificación de pares de bytes aplicado a una secuencia de caracteres.~\cite{Gage_Compresion}}{0.75}
...
\imagen{self-attention}{Cómo funciona self-attention: se calculan las puntuaciones de atención entre ``estación'' y cualquier otra palabra de la secuencia, y luego se utilizan para ponderar una suma de vectores de palabras que se convierte en el nuevo vector ``estación''.~\cite{DeepLearning}}{1}
...
Los \acrlong{llm} pueden clasificarse a grandes rasgos en tres tipos: modelos de preentrenamiento, modelos de \textit{Fine-tuning} y modelos multimodales~\cite{scribbleData}, ver figura~\ref{fig:TiposLLM}.
...
\imagen{TiposLLM}{Clasificación de los LLM en tres categorías.~\cite{scribbleData}}{1}
...
\imagen{OpenAI}{Evolución tecnológica de los modelos GPT a lo largo de los años.~\cite{zhao2023survey}}{1}
...
\imagen{LLaMa}{Gráfico de la evolución de los modelos LLaMa.~\cite{zhao2023survey}}{1}
...
\imagen{Mistral_Comparacion}{Resultados en MMLU, Razonamiento de sentido común, Conocimiento del mundo y Comprensión lectora para Mistral 7B y Llama 2 (7B/13/70B).~\cite{MiitaMistral}}{1}
...
\imagen{EsquemaRAG}{Secuencia para la creación de un \textit{Retrieval-augmented Generation}.~\cite{DeepLearning}}{1}
...
\imagen{rag}{Esquema del funcionamiento de un \textit{Retrieval-augmented Generation}.~\cite{LangchainRAG}}{1}
...
\imagen{VectorDatabase}{Esquema del funcionamiento de creación de bases de datos vectoriales con Embeddings.~\cite{DeepLearning}}{1}
...
\imagen{ModeloGestionTFGTFM}{Proceso de gestión de TFG en la UBU.~\cite{ubu-tfg}}{1}
...
\subsubsection{LLMs (Large Language Models)}
\begin{enumerate}
...
En la tabla \ref{tabla:comparativaplataformas} se muestra la comparativa entre las distintas plataformas y herramientas para la creación de chatbots.
...
\imagen{UBU-Chatbot}{UI del UBU-CHATBOT actual.~\cite{UBU-Chatbot}}{0.5}
...
\imagen{UBU-Chatbot2}{Limitaciones del UBU-CHATBOT actual relativas a la formulación de preguntas por parte de los estudiantes.~\cite{UBU-Chatbot}}{0.5}
